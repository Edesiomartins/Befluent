...
from app.services.progress import compute_streak, format_minutes
...
def test_session_minutes_uses_ended_at():
    ...
    assert session_minutes_for_progress(session) == 17


def test_active_session_does_not_count():
    start = datetime(2026, 7, 29, 10, 0, tzinfo=timezone.utc)
    session = StudySession(user_language_id="ul", started_at=start, ended_at=None, status="active")
    assert session_minutes_for_progress(session) == 0
